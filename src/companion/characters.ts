...
/** DiceBear PNG endpoint for dev — the placeholder avatars while the real
 *  image pipeline (DALL-E 3 / Imagen 3 using `buildImagePrompt(c)`) is
 *  not yet wired. "notionists" reads professional and adult; "personas"
 *  came in too cute (closed-eye sad faces, surprised mouths) for a career
 *  product. Production swaps the whole function for stable CDN URLs of
 *  the actually-generated Disney-Pixar portraits. */
export function avatarUrl(c: Character, size = 400): string {
  return (
    `https://api.dicebear.com/9.x/notionists/png?seed=${encodeURIComponent(c.seed)}` +
    `&size=${size}&backgroundColor=f1e5d6,f8f7f4&backgroundType=solid`
  );
}
